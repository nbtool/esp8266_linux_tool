...
XTENSA_LX106_ELF_LINK=https://dl.espressif.com/dl/xtensa-lx106-elf-linux64-1.22.0-92-g8facf4c-5.2.0.tar.gz
#ESP_IDF_LINK=https://github.com/espressif/esp-idf.git   
ESP_IDF_LINK=https://github.com/espressif/ESP8266_RTOS_SDK.git

#--------------------------------------------------------------------------
function install_tool_chain(){
    echo "> install tool chain ..."
    echo "> web page: https://docs.espressif.com/projects/esp8266-rtos-sdk/en/latest/get-started/linux-setup.html"
    if [ ! -d $XTENSA_LX106_ELF_PATH ]; then
        wget $XTENSA_LX106_ELF_LINK 
        tar -xzf xtensa-lx106-elf*.tar.gz
...
function install_esp_idf(){
    ...
    echo "> web page: https://github.com/espressif/ESP8266_RTOS_SDK"
    if [ ! -d $ESP_IDF_PATH ]; then
        git clone $ESP_IDF_LINK 
        mv ESP8266_RTOS_SDK $SDK_PATH/
    fi
}
