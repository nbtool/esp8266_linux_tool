...
function install_project_from_github(){
    echo "> install project form github ..."
        ...
        # remove the mqtt_atomizer.c
        # move cp mqtt_atomizer.c replace pre-one
        #rm -rf ./esp-aliyun/examples/mqtt/mqtt_atomizer/main/mqtt_atomizer.c 
        #cp ./my_src/mqtt_atomizer.c ./esp-aliyun/examples/mqtt/mqtt_atomizer/main/ 
    #fi
...
function app_op(){
    ...
    cd ./esp-aliyun/examples/ota/ota_example_mqtt/
    if [ "$1" == "defconfig" ]; then
        make defconfig
    elif [ "$1" == "config" ]; then
        make menuconfig
